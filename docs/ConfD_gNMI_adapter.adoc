...
[source, shell]
----
usage: confd_gnmi_server.py [-h] [-v] [-t {api,demo,nso}]
                            [--logging {error,warning,info,debug}]
                            [--insecure] [--port PORT] [--host HOST]
                            [-d {trace,debug,silent,proto}]
                            [--confd-addr CONFD_ADDR] [--api-port API_PORT]
                            [--monitor-external-changes]
...
options:
  ...
  -t {api,demo,nso}, --type {api,demo,nso}
                        gNMI server type
  --logging {error,warning,info,debug}
                        Logging level
  ...
  --confd-addr CONFD_ADDR
                        ConfD IP address (default is 127.0.0.1)
  --api-port API_PORT   API port (ConfD or NSO)
  --monitor-external-changes
                        start external changes service
  --external-port EXTERNAL_PORT
                        Port of external changes service (default is 5055)
...
We can run server in demo mode type (pass `-t demo`) or in API (`maapi`) mode against ConfD (pass `-t api`) or NSO (pass `-t nso`). Other modes (like `netconf` are currently not supported). For `demo` mode type, it may be necessary to pass config file (e.g. for `STREAM` subscriptions, `--cfg=data/demo.xml). We can pass
