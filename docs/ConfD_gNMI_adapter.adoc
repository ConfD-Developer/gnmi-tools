...
:gitplant: http://www.plantuml.com/plantuml/proxy?src=https://raw.githubusercontent.com/ConfD-Developer/gnmi-tools/main/docs
:gitplant-develop: http://www.plantuml.com/plantuml/proxy?src=https://raw.githubusercontent.com/micnovak/ConfD-Demos/gnmi-tools/develop/docs
...
----
$ ./src/confd_gnmi_server.py -h
usage: confd_gnmi_server.py [-h] [-v] [-t {demo,confd,nso}]
                            [--logging {error,warning,info,debug}] [--insecure]
                            [--port PORT] [--host HOST] [-d {trace,debug,silent,proto}]
                            [--confd-nso-addr CONFD_NSO_ADDR]
                            [--confd-nso-port CONFD_NSO_PORT]
                            [--monitor-external-changes] [--external-port EXTERNAL_PORT]
                            [--cfg CFG] [--key KEY] [--crt CRT]
...
options:
  ...
  -t {demo,confd,nso}, --type {demo,confd,nso}
                        gNMI server type
  ...
  -d {trace,debug,silent,proto}, --confd-nso-debug {trace,debug,silent,proto}
                        ConfD or NSO debug level
  --confd-nso-addr CONFD_NSO_ADDR
                        ConfD or NSO IP address (default is 127.0.0.1)
  --confd-nso-port CONFD_NSO_PORT
                        IPC port (ConfD or NSO)
  --monitor-external-changes
                        start external changes service
  --external-port EXTERNAL_PORT
                        Port of external changes service (default is 5055)
...
We can run server in demo mode type (pass `-t demo`) or in the mode against ConfD (pass `-t confd`) or NSO (pass `-t nso`). Other modes (like `netconf` are currently not supported). For `demo` mode type, it may be necessary to pass config file (e.g. for `STREAM` subscriptions, `--cfg=data/demo.xml). We can pass
...
NOTE: To switch between `confd` and `nso`, you need to first `unset PYTHONPATH`, source `confdrc` or `ncsrc` and start server in the appropriate mode. E.g.: `unset PYTHONPATH; source $CONFD_DIR/confdrc; ./src/confd_gnmi_server.py -t confd` or `unset PYTHONPATH; source $NCS_DIR/ncsrc; ./src/confd_gnmi_server.py -t nso`
...
[source, shell]
----
 $ ./src/confd_gnmi_client.py --help
usage: confd_gnmi_client.py [-h] [--version]
                            [-o {capabilities,set,get,delete,subscribe}]
                            [--logging {error,warning,info,debug}] [--insecure]
                            [--port PORT] [--host HOST] [--prefix PREFIX]
                            [-p PATHS] [-t {ALL,CONFIG,STATE,OPERATIONAL}]
                            [-v VALS] [-s {ONCE,POLL,STREAM}]
                            [--poll-count POLLCOUNT]
                            [--poll-interval POLLINTERVAL]
                            [--sample-interval SAMPLEINTERVAL]
                            [--interactive-poll] [--allow-aggregation]
...
options:
  ...
  -t {ALL,CONFIG,STATE,OPERATIONAL}, --data-type {ALL,CONFIG,STATE,OPERATIONAL}
                        'data type' for get operation (default 'ALL')
  ...
  --poll-interval POLLINTERVAL
                        ...
                        0.5)
  --sample-interval SAMPLEINTERVAL
                        Interval (in ms) between SAMPLE requests in STREAM
                        subscription mode(if set, SAMPLE Subscriptions are
                        created)
  --interactive-poll    Poll subscription invoked interactively.
  --allow-aggregation   Set allow aggregation option when invoking subscription.
  --subscription-end-delay SUBSCRIPTION_END_DELAY
  ...
  --encoding {JSON,JSON_IETF,PROTO}
                        Requested encoding for get and subscribe (default
                        'JSON_IETF')
----
...
.get values for `python-vm` from NSO when started in `-t nso` mode type
[source, shell, role="acopy"]
----
./src/confd_gnmi_client.py -o get --path /python-vm
----
...
NOTE: Following subscribe examples use `ONCE` (default) subscription mode. It is possible to use `POLL` (with `--poll-count` and `--poll-interval`) and `STREAM` (with `--read-count`) mode as well. `STREAM` subscription with `SAMPLE` mode can be invoked with `--sample-interval` parameter.
...
State (operational) data can be stored in the ConfD/NSO CDB, or they can be provided
to ConfD/NSO by external application (called data provider).
In the first case (data stored in the CDB) the ConfD/NSO can monitor itself the
...
NOTE: This functionality is implemented only for ConfD or NSO API adapter type (use `-t confd` or `-t nso` when starting the gNMI Adapter server).
...
randomly generates changes.  Data provider is source of the data, not the CDB.
...
[source, shell, role="acopy"]
----
./src/confd_gnmi_server.py -t confd --monitor-external-changes
----
...
* GnmiConfDApiServerAdapter - adapter using ConfD/NSO
...
NOTE: Currently, this functionality is implemented only in ConfD/NSO Adapter subcomponent
(`-t confd` or `-t nso`).
