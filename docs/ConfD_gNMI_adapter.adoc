...
NOTE:: To switch between `confd` and `nso`, you need to first `unset PYTHONPATH`, source `confdrc` or `ncsrc` and start server in the appropriate mode. E.g.: `unset PYTHONPATH; source $CONFD_DIR/confdrc` and `unset PYTHONPATH; source $NCS_DIR/ncsrc`
